...
st.set_page_config('Credit Agricole deals',layout='wide')

st.title('Credit Agricole project finance deals')

### Infralogic data

# maxidf = pd.DataFrame()
#
# for i in [str(j) for j in range(1,17)]:
...
def getdata():
    df = pd.read_excel('ca_deals.xlsx').iloc[:,1:]
...
yearselect = st.slider('Select period',minyear,maxyear,(2018,maxyear))
...
fig = px.treemap(ca,path=order,values='Ticket USD',color='region')
st.plotly_chart(fig)
...
for sector in sectors:
    try:
        ...
        ca = ca[['Name','Ticket USD']]
        ca = pd.DataFrame(ca.groupby('Name').sum())
